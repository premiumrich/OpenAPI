...
describe Trulioo::DataFields do
  let(:model) { Trulioo::DataFields.new }
  let(:test_attributes) {
    {
  ...
  it "should have correct array of acceptable JSON keys" do
    expect(Trulioo::DataFields.acceptable_attributes).to eql([
      :"PersonInfo",
      :"Location",
      :"Communication",
      :"DriverLicence",
  ...
  it "should have correct array of acceptable attribute types" do
    expect(Trulioo::DataFields.openapi_types.values).to eql([
      :"PersonInfo",
      :"Location",
      :"Communication",
      :"DriverLicence",
  ...
  it "should have correct set of nullable attributes" do
    expect(Trulioo::DataFields.openapi_nullable).to eql(Set.new([]))
  end
  ...
  it "should correctly initialize with attributes from Hash" do
    model = Trulioo::DataFields.new(test_attributes)
  ...
  it "should not initialize with a non-Hash argument" do
    expect { Trulioo::DataFields.new([]) }.to raise_error(ArgumentError)
  end

  it "should not initialize with a Hash that contains an invalid attribute" do
    expect { Trulioo::DataFields.new({ test_invalid_key: "" }) }.to raise_error(ArgumentError)
  end

  it "should correctly check object equality" do
    model.person_info = test_attributes[:person_info]

    expect(model).to equal(model)
    expect(model).to eql(Trulioo::DataFields.new({ person_info: test_attributes[:person_info] }))
  end

  it "should correctly calculate hash code" do
    model.person_info = test_attributes[:person_info]

    expect(Trulioo::DataFields.new({ person_info: test_attributes[:person_info] }).hash).to eql(model.hash)
  end
end
